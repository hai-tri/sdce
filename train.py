...
class Config:
    ...
    def from_args(cls, args: argparse.Namespace) -> "Config":
        ...
        if hasattr(args, 'standard_training') and args.standard_training:
            ...
            # If standard loss type, disable surrogate
...
def surrogate_cross_entropy_loss(
    ...
    if perp_indices is not None and lookup_surrogate_to_self is not None and surrogate_weight > 0:
        ...
        # Mask invalid entries before softmax by setting to inf
...
class SurrogateTrainer:
    ...
    def compute_surrogate_guidance(
        ...
        with torch.no_grad():
            ...
            surrogate_perp_full = torch.reciprocal(F.softmax(surrogate_logits, dim=-1) + 1e-8)
...
def _train_fn(rank: int, config: Config, base_tokenizer, surrogate_tokenizer, train_dataset, eval_dataset):
    ...
    # Set seed without rank offset for identical model initialization
    ...
    # Set rank-specific seed after model init for data shuffling and dropout
...
def main():
    ...
    if args.config:
        ...
        if hasattr(args, 'standard_training') and args.standard_training:
            ...
            if args.loss_type == "standard":
                config.surrogate.enabled = False
        # Handle --no_surrogate flag
        if hasattr(args, 'no_surrogate') and args.no_surrogate:
            config.surrogate.enabled = False
            if config.training.loss_type != "standard":
                config.training.loss_type = "standard"
    else:
    ...
    # Set random seed (same for all ranks to ensure identical model initialization)
    set_seed(config.training.seed)
