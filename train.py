...
def surrogate_cross_entropy_loss(
    ...
    Returns:
        ...
        - z_loss may be None if compute_z_loss=False
    ...
    if perp_indices is not None and lookup_surrogate_to_self is not None and surrogate_weight > 0:
        ...
        # Gather log probabilities for the top-k tokens
        ...
        # CRITICAL: Use float32 for numerical stability (avoid NaN on MPS/float16)
        full_log_probs = F.log_softmax(logits.float(), dim=-1)
        gathered_log_probs = torch.gather(full_log_probs, dim=2, index=translated_perp_indices)
        gathered_nll = -gathered_log_probs  # Negative log likelihood (keep in float32)
        ...
        # CRITICAL: Compute softmax in float32 to avoid NaN from -inf inputs
        softmax_weights = F.softmax(-masked_perp_values.float(), dim=-1)
        ...
        # CRITICAL: We must handle inf * 0 = NaN case.
        # gathered_nll can be inf for very low probability tokens (log(~0) = -inf, so -log = +inf)
        # Even though softmax_weights zeros out invalid positions, inf * 0 = NaN.
        # Solution: Zero out gathered_nll where weights are zero BEFORE multiplication.
        gathered_nll_safe = torch.where(
            softmax_weights > 0,
            gathered_nll,
            torch.zeros_like(gathered_nll)
        )
...
def kl_divergence_loss(
    ...
    """
    Combined cross-entropy and KL divergence loss for knowledge distillation.
    
    The total loss is: CE_loss + kl_weight * KL(teacher || student)
    
    Args:
        student_logits: Student model logits (batch_size, seq_len, vocab_size)
        teacher_logits: Teacher/surrogate model logits (batch_size, seq_len, vocab_size)
    ...
    if student_logits.dim() == 2:
        # Try to infer from teacher_logits
        if teacher_logits.dim() == 3:
            batch_size, seq_len, vocab_size = teacher_logits.shape
            student_logits = student_logits.view(batch_size, seq_len, -1)
            labels = labels.view(batch_size, seq_len)
    
    batch_size, seq_len, student_vocab_size = student_logits.shape
    teacher_vocab_size = teacher_logits.shape[-1]
    
    # Handle vocabulary size mismatch - use minimum vocab size
    min_vocab_size = min(student_vocab_size, teacher_vocab_size)
    student_logits_aligned = student_logits[..., :min_vocab_size]
    teacher_logits_aligned = teacher_logits[..., :min_vocab_size]
    
    # Create mask for valid positions (not padding)
    valid_mask = (labels != ignore_index)  # (batch, seq)
    
    # Standard cross-entropy loss on hard labels
    student_logits_flat = student_logits.view(-1, student_vocab_size)
    labels_flat = labels.view(-1)
    ce_loss = F.cross_entropy(student_logits_flat, labels_flat, ignore_index=ignore_index, reduction='none')
    ...
    # CRITICAL: Clamp teacher_probs to avoid 0 * log(0) = 0 * -inf = NaN in KL computation
    # When teacher_probs is 0 for some tokens, the contribution should be 0, but
    # F.kl_div computes p * (log(p) - log(q)) which gives 0 * -inf = NaN
    # Clamping to a small epsilon ensures numerical stability
    teacher_probs = teacher_probs.clamp(min=1e-8)
    
    # Also ensure student_log_probs doesn't have -inf (though log_softmax is usually stable)
    student_log_probs = student_log_probs.clamp(min=-100)  # log(1e-44) ≈ -100
    
    # KL divergence per position: sum over vocab dimension
    # Using the formula: KL = sum(p * log(p)) - sum(p * log(q))
    # Which is: -entropy(teacher) - sum(teacher * student_log_probs)
    # Simplified: sum(teacher * (log(teacher) - log(student)))
    ...
    # Scale KL by temperature^2 (standard practice in distillation)
    kl_per_position = kl_per_position * (temperature ** 2)
    
    # Mask invalid positions
    ce_loss = ce_loss * valid_mask.float()
    kl_per_position = kl_per_position * valid_mask.float()
    
    # Combine losses
    num_valid = valid_mask.sum().item()
    
    if reduction == "mean":
        ce_loss_reduced = ce_loss.sum() / (num_valid + 1e-8)
        kl_loss_reduced = kl_per_position.sum() / (num_valid + 1e-8)
        loss = ce_loss_reduced + kl_weight * kl_loss_reduced
    elif reduction == "sum":
        loss = ce_loss.sum() + kl_weight * kl_per_position.sum()
    else:  # reduction == "none"
        loss = ce_loss + kl_weight * kl_per_position
    
    # Compute z-loss if requested
    z_loss = None
    if compute_z_loss:
        z_squared = student_logits.view(-1, student_vocab_size).logsumexp(-1).pow(2)
        label_mask = (labels.view(-1) != ignore_index).float()
...
class SurrogateTrainer:
    ...
    def compute_surrogate_guidance(
        ...
        with torch.no_grad():
            ...
            # === GATHER PERPLEXITY VALUES FOR SELECTED TOKENS ===
    ...
    def train_step(
        ...
        if self.use_autocast and not self.is_tpu:
            ...
            # TPU bfloat16 - use autocast if available in torch_xla (requires PyTorch XLA 2.0+)
            try:
                autocast_context = torch.autocast('xla', dtype=torch.bfloat16, enabled=True)
            except (ValueError, RuntimeError):
                # Older PyTorch XLA versions don't support torch.autocast('xla', ...)
...
def _setup_tpu_pod_environment(config: Config) -> Tuple[bool, int, int]:
    ...
    if is_pjrt and HAS_XLA_RUNTIME and config.training.tpu_use_pjrt:
        logger.info("PJRT runtime detected for TPU Pod training")
