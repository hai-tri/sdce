...
def cross_entropy_loss(
    ...
    if perp_indices is not None and lookup_surrogate_to_self_tokens is not None and surrogate_weight > 0:
        ...
        # combined_mask afterwards which zeros out these invalid rows anyway.
        ...
        row_has_valid = combined_mask.any(dim=-1, keepdim=True).expand_as(masked_perp)
        masked_perp = torch.where(row_has_valid, masked_perp, torch.zeros_like(masked_perp))
        
        # Softmax over k dimension (the top-k tokens)
        # CRITICAL: Compute in float32 to avoid NaN from -inf inputs on MPS/float16
        softmax_weights = F.softmax(-masked_perp.float(), dim=-1)
...
def compute_perplexity_guidance(
    ...
    # CRITICAL: Compute in float32 with larger epsilon to avoid overflow on MPS/float16
    surrogate_probs = F.softmax(surrogate_logits.float(), dim=-1)
    surrogate_perp = torch.reciprocal(surrogate_probs + 1e-6)
    # Clamp to avoid extreme values
    surrogate_perp = surrogate_perp.clamp(max=1e6)
